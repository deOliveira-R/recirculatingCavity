...
#    return list(itertools.chain(*l))
    ...
    #print([element for tupl in l for element in tupl])
    
    #return l.flatten()
#    result = []
#    print("Testing list:", l)
#    for el in l:
#        print("Testing element: ", el, type(el))
...
class Tensor:
    ...
    def determinant(self):
        if NP.shape(self.array) == (3, 3):
            ...
            raise ValueError('Tensor must of rank 2 and shape 3 by 3')
    ...
    def matrixmultiply(self, other):
        if self.rank == 2 and other.rank == 2:
            ...
            raise ValueError('Tensor must of rank 2')
...
def div(F):
    ...
    if F.rank == 0:
        ...
        rank = F.rank - 1
        print(rank)
        divF = NP.array([X]*(3**rank))
        print("The allocated divF is:\n", divF)
        print("The tensor F is:\n", F)
        flatF = NP.resize(F.array, flatten((F.array.shape, 3)))
        print("The flat tensor is: ", flatF)

        for r in range(rank + 1):
            divF[r] = sum(sympy.diff(flatF[r*i+i], coord) for i, coord in enumerate(varList))
        if rank > 0:
            divF.shape = [3, ]*rank
        return Tensor(divF)
